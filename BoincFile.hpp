...
class BoincFile {
public:
	bool open(std::string, std::string); // Opens a file within BOINC. // args: path(string), mode(string)
	bool close(); // Closes a previously opened file.
	bool getLine(std::string&); // Gets the next line of the opened file.
	bool write(std::string); // Writes the given string into a file within BOINC.
